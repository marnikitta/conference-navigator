...
import { dayLabel, tierText } from "@/composables/usePapers";
import type { Paper } from "@/types";

const ui = useUiStore();
const papersStore = usePapersStore();
const saved = useSavedStore();
const { openPaperId } = storeToRefs(ui);
const { papers } = storeToRefs(papersStore);

const paper = computed<Paper | null>(() => {
  if (!openPaperId.value) return null;
  return papers.value.find((p) => p.id === openPaperId.value) || null;
});
...
const whenLabel = computed(() => {
  const p = paper.value;
  if (!p || !p.day) return "";
  return `${dayLabel(p.day)} · ${p.start}–${p.end}`;
});
        ...
        <template v-if="similar.length">
          ...
          >
            <div>
              ...
              <div class="sim-meta">
                {{ tierText(s) }}
                <template v-if="s.day"> · {{ s.day }} {{ s.start }} </template>
